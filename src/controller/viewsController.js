...
export const renderUserDashboard = async (req, res) => {
    try {
        ...
        const profileLink = `http://${config.railway_public_domain}/current`;
        const cartLink = `http://${config.railway_public_domain}/carts`
        const {prevPage, nextPage, hasPrevPage, hasNextPage} = products;
        const prevLink = hasPrevPage ? `http://${config.railway_public_domain}/products?page=${prevPage}` : null;
        const nextLink = hasNextPage ? `http://${config.railway_public_domain}/products?page=${nextPage}` : null;
...
export const renderPremiumDashboard = async (req, res) => {
    try {
        ...
        const cartLink = `http://${config.railway_public_domain}/carts`
        const {prevPage, nextPage, hasPrevPage, hasNextPage} = products;
        const prevLink = hasPrevPage ? `http://${config.railway_public_domain}/realtimeproducts?page=${prevPage}` : null;
        const nextLink = hasNextPage ? `http://${config.railway_public_domain}/realtimeproducts?page=${nextPage}` : null;
...
export const renderAdminDashboard = async (req, res) => {
    try {
        ...
        const {limit, page, sort, query} = req.query;
        const products = await getProductsService(limit, page, sort, query);
        const userList = `http://${config.railway_public_domain}/admindashboard/userlist`;
        const {prevPage, nextPage, hasPrevPage, hasNextPage} = products;
        const prevLink = hasPrevPage ? `http://${config.railway_public_domain}/admindashboard?page=${prevPage}` : null;
        const nextLink = hasNextPage ? `http://${config.railway_public_domain}/admindashboard?page=${nextPage}` : null;
...
export const userList = async (req, res) => {
    try {
        ...
        const dashboardLink = `http://${config.railway_public_domain}/admindashboard?page=1`       
...
export const renderProfile = async (req, res) => {
    try {
        ...
        const dashboardLink = `http://${config.railway_public_domain}/products?page=1`;
